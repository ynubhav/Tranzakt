...
userrouter.put("/updateinfo", authmiddleware, async (req, res) => {
  ...
  if (updateinfo) res.status(200).json({ message: "Updated successfully" });
  else res.status(411).json({ message: "Error while updating information" });
});
...
userrouter.post("/friends", authmiddleware, async (req, res) => {
  try {
    ...
    if (action === "find") {
      //find all friends username and firstname and id
    ...
    if (action === "add") {
      ...
      const x = await User.findByIdAndUpdate(id, {
    ...
    if (action === "remove") {
      const y = await User.findByIdAndUpdate(id, {
