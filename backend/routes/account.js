...
accountrouter.post("/transfer", authmiddleware, async (req, res) => {
  ...
  try {
    ...
    const account = await Accounts.findOne({ userId: fromuserid }).session(
      session
    );
    const balance = account ? account.balance : 0;
    const senderexists = await Accounts.findOne({ userId: fromuserid }).session(
      session
    );
    const recieverexists = await Accounts.findOne({ userId: touserid }).session(
      session
    );
    if (amount > balance || !account || amount < 0) {
      ...
      return res
        .status(400)
        .json({ message: "Insufficient balance / Invalid SENDER" });
    }
    if (!senderexists || !recieverexists) {
      await session.abortTransaction();
      return res.status(400).json({ message: "Invalid account(s)" });
    }
    const x = await Accounts.findOneAndUpdate(
      { userId: fromuserid },
      { $inc: { balance: -amount }, $push: { transctions: fromTransaction } },
      { session }
    );
    const y = await Accounts.findOneAndUpdate(
      { userId: touserid },
      { $inc: { balance: amount }, $push: { transctions: toTransaction } },
      { session }
    );
